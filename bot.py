...
ADMIN_TITLES = {
    ...
    858261251: "тренерки з продукту Анастасії",
...
async def cmd_savepost(message: Message, state: FSMContext):
    ...
    # ❗ ФІКС: ЖОРСТКЕ ОЧИЩЕННЯ СТАНУ ПЕРЕД ПОЧАТКОМ ❗
    await state.clear() 

    await state.set_state(BroadcastStates.waiting_for_content)
    await state.update_data(is_silent_mode=True) # ЯВНО ВМИКАЄМО ТИХИЙ РЕЖИМ
...
async def cmd_broadcast(message: Message, state: FSMContext):
    ...
    await state.set_state(BroadcastStates.waiting_for_content)
    await state.update_data(is_silent_mode=False) # ЯВНО ВИМИКАЄМО ТИХИЙ РЕЖИМ
...
---
**🏷️ Керування Мітками (Тегами):** (Працює у відповідь на тікет)

* **➕ Додати мітку:** Відповідайте (Reply), починаючи з **#tag_[МІТКА]** *(Приклад: `#tag_КМ_535 Це відповідь`)*
* **❌ Видалити мітку:** Відповідайте (Reply), починаючи з **#del_[МІТКА]**
  *(Приклад: `#del_КМ_535 Видаляю мітку`)*
* **🗑️ Очистити всі мітки:** Відповідайте (Reply), починаючи з **#set_**
  *(Приклад: `#set_ Усі мітки видалено`)*
    """
